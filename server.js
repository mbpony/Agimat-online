...
function validateSaveGold(rec, saveStr){
  ...
  /* PHASE 8 — escrow strip: items currently held by the Palengke (rec.esc)
     must not exist in the player's save. Kills the post-then-restore-backup
     duplication exploit. Match uid+name so recycled uids of new items survive. */
  const esc=rec.esc||{}, stripped=[];
  if(Object.keys(esc).length){
    if(Array.isArray(sv.bag)){
      sv.bag=sv.bag.filter(it=>{
        if(it&&esc[it.uid]&&esc[it.uid].name===it.name){ stripped.push(it.uid); return false; }
        return true;
      });
    }
    if(sv.gear&&typeof sv.gear==='object'){
      for(const k of Object.keys(sv.gear)){
        const it=sv.gear[k];
        if(it&&esc[it.uid]&&esc[it.uid].name===it.name){ stripped.push(it.uid); delete sv.gear[k]; }
      }
    }
    if(stripped.length){ mutated=true; g.flags=(g.flags||0)+1; }
  }
  return {save:(clamped||mutated)?JSON.stringify(sv):saveStr, clamped, gold:sv.gold, stripped};
}
  ...
  jsonBody(req,(b)=>{
    ...
    if(p==='/api/save'){
      ...
      if(typeof b.save==='string' && b.save.length<400000){
        ...
        if(v.clamped||v.stripped.length){
          const out={ok:true};
          if(v.clamped){ out.clamped=true; out.gold=v.gold; }
          if(v.stripped.length) out.escStripped=v.stripped;
    ...
    if(p==='/api/market'){
      ...
      if(act==='post'){
        ...
        /* PHASE 8 §25 — ownership check: the item must exist in the seller's
           STORED save (bag or gear), matched by uid + identity fields. The
           SERVER's copy is what gets listed (client-sent stats are ignored →
           affix tampering dies here). */
        const sv=parseSave(rec);
        const uid=b.item.uid;
        const matches=(it)=>it&&it.uid===uid&&it.name===b.item.name&&it.rarity===b.item.rarity&&(it.ilvl|0)===(b.item.ilvl|0);
        let owned=null, bagIdx=-1, gearSlot=null;
        if(Array.isArray(sv.bag)){ bagIdx=sv.bag.findIndex(matches); if(bagIdx>=0) owned=sv.bag[bagIdx]; }
        if(!owned&&sv.gear&&typeof sv.gear==='object'){
          for(const k of Object.keys(sv.gear)) if(matches(sv.gear[k])){ owned=sv.gear[k]; gearSlot=k; break; }
        }
        if(!owned) return sendJSON(res,404,{ok:false,err:'Hindi makita ang gamit sa iyong naka-imbak na save — i-refresh ang laro at subukan muli.'});
        /* escrow: item leaves the stored save NOW and is remembered in rec.esc */
        if(bagIdx>=0) sv.bag.splice(bagIdx,1); else delete sv.gear[gearSlot];
        rec.save=JSON.stringify(sv);
        rec.esc=rec.esc||{}; rec.esc[uid]={name:owned.name, ts:Date.now()};
        const id=MARKET.nextId++;
        MARKET.listings.push({id, seller:me, sellerName:rec.display, item:owned, price, ts:Date.now()});
        persistMarket(); persistUsers();
      ...
      if(act==='buy'){
        ...
        if(srec){
          srec.mail=srec.mail||[]; srec.mail.push({gold:net, note:`Nabenta: ${L.item.name||'gamit'} (−5% buwis)`, ts:Date.now()});
          if(srec.esc&&L.item&&L.item.uid!=null) delete srec.esc[L.item.uid];   // PHASE 8: escrow released (sold)
          persistUsers();
        }
        persistMarket();
      ...
      if(act==='cancel'){
        ...
        if(rec.esc&&L.item&&L.item.uid!=null){ delete rec.esc[L.item.uid]; persistUsers(); }   // PHASE 8: escrow released (returned)
    ...
    if(p==='/api/arena'){
      ...
      if(b.action==='report'){
        ...
        /* PHASE 9a §25 step 3 — SERVER-ISSUED arena purse. The client never
           adds arena gold itself; the server mails it (mail_claim credits the
           gold ledger, so the payout is also cheat-proof on the save side).
           Daily cap: first 5 wins per UTC day pay a purse. */
        let purse=0;
        if(b.win){
          rec.arena.r+=25; rec.arena.w++;
          const today=dayKey(now);
          if(rec.arena.purseDay!==today){ rec.arena.purseDay=today; rec.arena.purseN=0; }
          if((rec.arena.purseN||0)<5){
            rec.arena.purseN=(rec.arena.purseN||0)+1;
            purse=300+Math.min(700,Math.floor(rec.arena.r/10));   // 300..1000 by rating
            rec.mail=rec.mail||[];
            rec.mail.push({gold:purse, note:`Premyo ng Arena — panalo #${rec.arena.purseN} ngayong araw`, ts:now});
          }
        } else { rec.arena.r=Math.max(0,rec.arena.r-20); rec.arena.l++; }
        persistUsers();
        return sendJSON(res,200,{ok:true, arena:rec.arena, purse});
      }
      if(b.action==='top'){
        const rows=[];
