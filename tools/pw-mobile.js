...
(async()=>{
  ...
  const ctx=await browser.newContext({viewport:{width:360,height:740},hasTouch:true,isMobile:true,deviceScaleFactor:1,userAgent:'Mozilla/5.0 (Linux; Android 10; SM-A107F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120 Mobile Safari/537.36 UCBrowser/13 Quetta'});
  ...
  try{ await page.waitForFunction(()=>window.MAPS&&window.MAPS.length>0,{timeout:70000}); }catch(e){ console.log('NO MAPS (mobile) errs='+JSON.stringify(errs)); const st=await page.evaluate(()=>({title:document.title,three:typeof THREE,maps:typeof window.MAPS,scripts:document.scripts.length})); console.log('state='+JSON.stringify(st)); }
