// Round-trip the Map Instance Manager (Phase 1). Local only.
...
(async()=>{
  ...
  console.log('start:', JSON.stringify(await p.evaluate(()=>window._dbgMap())));
  await p.evaluate(()=>window.changeMap('map_test'));
  const s1=await waitMap('map_test');
  console.log('after ->map_test:', JSON.stringify(s1));
  await p.screenshot({ path:'docs/instance-test.png' });

  await p.evaluate(()=>window.changeMap('map_starting'));
  const s2=await waitMap('map_starting');
  console.log('after ->map_starting:', JSON.stringify(s2));
