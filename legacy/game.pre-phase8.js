...
function acctSetSession(token,user,color){
...
function hitEnemy(en, mult, opts={}){
  ...
  if(opts.stun) en.stunUntil=nowS()+opts.stun;
  if(opts.knock){
...
function showWelcome(){
  ...
  function setMode(m2){
    ...
    $('acct-go').textContent = m2==='login' ? '🔑 MAG-LOG IN' : '✨ GUMAWA NG ACCOUNT';
  ...
  function showLogged(){
    ...
    $('acct-logged').classList.toggle('hidden',!has);
    $('acct-skip').classList.toggle('hidden',has);
    if(has) $('acct-who').textContent='🧿 Naka-log in: '+ACCT.user;
  }
  ...
  $('acct-go').onclick=async ()=>{
    ...
    if(!u||!pw){ msg.textContent='Ilagay ang username at password.'; msg.className=''; return; }
    $('acct-go').disabled=true; msg.textContent='Sandali lang…'; msg.className='ok';
    ...
    acctSetSession(r.token,r.user,r.color);
    msg.textContent=''; showLogged();
    if(r.save){                                     // cloud save exists → jump straight in
      if(adoptSave(r.save) && S.cls){
        toast('☁️ Na-load ang iyong cloud save. Maligayang pagbabalik, '+(S.name||r.user)+'!','levelup');
        $('welcome-screen').classList.add('hidden');
        bootFromSave();
        return;
      }
    }
    toast(mode==='login'?'🔑 Naka-log in! Pindutin ang SIMULAN.':'✨ Gumawa ng account! Ang progreso mo ay mase-save sa cloud.','levelup');
  };
  ...
  $('acct-continue').onclick=async ()=>{
    ...
    if(r.ok&&r.save&&adoptSave(r.save)&&S.cls){
      ...
      toast('Wala pang cloud save — pindutin ang SIMULAN para magsimula!','warn');
    } else { acctLogout(); showLogged(); toast(r.err||'Nag-expire ang session.','warn'); }
  };
  $('acct-logout').onclick=()=>{ acctLogout(); showLogged(); setMode('login'); };
  setMode('login'); showLogged();
}
...
function ccRebuildMesh(){
  ...
  // strip the blob shadow for the pedestal view
  ccMesh.traverse(o=>{ if(o.isMesh&&o.material._isFx) o.visible=false; });
  ccScene.add(ccMesh);
}
function ccValidate(){
  const ok=charDraft.name.trim().length>=2 && !!charDraft.cls;
  $('cc-confirm').disabled=!ok;
  $('cc-confirm').textContent=ok?'⚔️ HANDA NA, '+charDraft.name.trim().toUpperCase()+'!':'⚔️ HANDA NA!';
}
function showCharCreate(){
  $('char-create').classList.remove('hidden');
  /* name */
  const nameEl=$('cc-name');
  nameEl.oninput=()=>{ charDraft.name=nameEl.value; ccValidate(); };
  $('cc-dice').onclick=()=>{
    const pool=FILIPINO_NAMES[charDraft.gender];
    charDraft.name=pool[Math.floor(Math.random()*pool.length)];
    nameEl.value=charDraft.name; ccValidate();
  };
  /* gender */
  ...
  for(const [id,C] of Object.entries(CLASSES)){
    const b=document.createElement('button');
    b.textContent=C.skills[0].icon+' '+CLASS_TAGALOG[id].tl;
    b.onclick=()=>{
      ...
      $('cc-classinfo').innerHTML=`<b style="color:var(--gold-hi)">${CLASS_TAGALOG[id].tl}</b> · ${C.role} · ${C.weapon}<br>${CLASS_TAGALOG[id].desc}<br>
        <span style="opacity:.8">${C.skills.map(s2=>s2.icon+' '+s2.name).join(' · ')}</span>
...
async function openMarket(tab){
  ...
  document.querySelectorAll('[data-post]').forEach(b=>b.onclick=async()=>{
    ...
    const idx=S.bag.findIndex(i=>i.uid===uid); if(idx<0) return;
    const it=S.bag[idx];
    S.bag.splice(idx,1); save();                       // escrow: item leaves the bag first
    const r=await api('/api/market',{token:ACCT.token, action:'post', item:it, price});
    if(r.ok){ toast(`🏷️ Nakatinda: <b>${it.name}</b> — 🪙${fmt(price)} (−5% buwis sa benta)`,'loot'); openMarket('mine'); }
    else { S.bag.splice(idx,0,it); save(); toast(r.err||'Hindi naitinda.','warn'); openMarket('sell'); } // refund on failure
  });
...
(function skillSystem(){
      ...
      case 'melee': {
        const run=()=>{ if(player.dead) return;
          ...
          for(const en of enemies){
            ...
            if(e.slow){ en._skSlowUntil=nowS()+e.slow[1]; en._skSlowPct=e.slow[0]; }
            if(e.root){ en.rootUntil=nowS()+rankUp(e.root,e.prRoot||0); }
            if(e.shred){ en._skShredUntil=nowS()+e.shred[1]; en._skShredPct=e.shred[0]; }
            if(e.dot){ en._skDot={kind:e.dot[0],dps:rankUp(e.dot[1],e.dot[2]),until:nowS()+e.dot[3]}; }
            if(e.kulam){ en.bleedUntil=nowS()+4; en.kulamUntil=nowS()+8; addLabel('🕷️','warn',en.x,groundY(en.x,en.z)+2.6,en.z,0.7); }
  ...
  hitEnemy=function(en,mult,opts={}){
    ...
    if(p){
      ...
      if(p._skSlow){ en._skSlowUntil=nowS()+p._skSlow[1]; en._skSlowPct=p._skSlow[0]; }
