...
- **customHeroRig module**: QUATERNION-ONLY retarget (rotation tracks bind by sanitized name; chibi rest positions kept → walang proportion distortion; hips.position ×0.69 height ratio para sa locomotion bounce; scale tracks dropped). Body+outfit = separately-skinned twins sa 2 mixers in lockstep. Hair + face plane attach sa HEAD BONE — sumusunod na sa animation. handslot.l/r present sa rig (weapon attach compatible via sanitize-tolerant findBone). State machine: moving→Running_A · attack→profile clip (casters nag-Spellcast!) · dodge · death; remote ka-party animated via movement deltas.
- **Fallback chain ngayon**: rigged custom → static composite → KayKit → procedural. rigHelpers: skinned-mesh skin tint, hair tint, face-on-bone bridge (_faceTexFor sa facePlanes).
- Headless 11/11 (build, lockstep mixers, retarget filter, hips scaling, face-on-bone, tint, full state machine, caster profile). Scope-gate clean. SW → agimat-v22.
- NEXT: outfit fitting refinement (user-approved queue).
